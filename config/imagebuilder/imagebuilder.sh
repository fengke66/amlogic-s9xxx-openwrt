...
rebuild_firmware() {
    ...
    # Cleaned packages query to maintain full compatibility with 25.12.0 APK branch without comment syntax error
    my_packages="\
        acpid attr base-files bash bc blkid block-mount blockd bsdtar btrfs-progs busybox bzip2 \
        cgi-io chattr containerd coremark coreutils coreutils-base64 coreutils-nohup coreutils-truncate \
        curl docker docker-compose dockerd dosfstools dumpe2fs e2freefrag e2fsprogs exfat-mkfs \
        f2fs-tools f2fsck fdisk gawk getopt git gzip hostapd-common iconv iw iwinfo jq jshn \
        kmod-brcmfmac kmod-brcmutil kmod-cfg80211 kmod-mac80211 libjson-script liblucihttp \
        losetup lsattr lsblk lscpu mkf2fs mount-utils openssl-util parted \
        perl-http-date perlbase-file perlbase-getopt perlbase-time perlbase-unicode perlbase-utf8 \
        pigz pv rename resize2fs runc tar tini ttyd tune2fs uclient-fetch uhttpd uhttpd-mod-ubus \
        unzip usb-modeswitch uuidgen wget-ssl whereis which wwan xfs-fsck xfs-mkfs xz xz-utils \
        ziptool zoneinfo-asia zoneinfo-core zstd kmod-ath10k kmod-ath10k-sdio ath10k-firmware-qca9377 \
        luci luci-base luci-compat luci-i18n-base-zh-cn luci-lib-base luci-lib-ip luci-lib-jsonc \
        luci-lib-nixio luci-mod-admin-full luci-mod-network luci-mod-status luci-mod-system \
        luci-proto-ipip luci-proto-ipv6 luci-proto-openconnect luci-proto-relay \
        luci-app-amlogic luci-i18n-amlogic-zh-cn ${config_list} \
        "
